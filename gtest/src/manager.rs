...
impl JournalHandler for ExtManager {
    ...
    fn update_page_data(&mut self, program_id: ProgramId, page: PageNumber, data: Vec<u8>) {
        let (actor, _) = self
            .actors
            .get_mut(&program_id)
            .expect("Can't find existing program");
        if let Some(pages_data) = actor.get_pages_data_mut() {
    ...
    fn update_persistent_pages(
        ...
        if let Actor::Initialized(Program::Genuine(program, pages_data)) = actor {
            ...
            *program.get_pages_mut() = allocations;
        } else {
            unreachable!("No pages update for non-initialized program")
        }
    }
